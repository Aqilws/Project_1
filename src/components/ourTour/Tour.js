        ...
        {Data.map((item) => (
          <div className="w-60 flex flex-col hover:skew-x-2 hover:scale-105">
            ...
            <div className="flex justify-around gap-2 mt-5">
              <Link to={`/view/${item.id}`}  className="w-full bg-blue-600 rounded-md font-semibold text-slate-200">View</Link>
              <Link to={`/edit/${item.id}`}  className="w-full bg-green-600 rounded-md font-semibold text-slate-200">Edit</Link>
              <button
                onClick={() => {
                  RemoveItem(item.id);
                }}
